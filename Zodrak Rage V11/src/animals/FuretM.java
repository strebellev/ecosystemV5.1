...
public class FuretM {
	...
	public FuretM(PVector l, DNAfuret dnafuret_, PApplet p){
		...
		lifeTime = PApplet.map(dnafuret.genes[0], 0, 1, 300, 400);
		maxLifeTime = PApplet.map(dnafuret.genes[0], 0, 1, 300, 400);
		//water = PApplet.map(dnafuret.genes[1], 0, 1, 100, 200);
		food = PApplet.map(dnafuret.genes[1], 0, 1, 200, 300);
		maxFood = PApplet.map(dnafuret.genes[1], 0, 1, 200, 300);
		fieldOfView = PApplet.map(dnafuret.genes[2], 0, 1, 300, 400);
		
		speed = PApplet.map(dnafuret.genes[0], 0, 1, 1, 2);
		
		///for food 1000 = 1kg
		weight = PApplet.map(dnafuret.genes[1], 0, 1, 1000, 2000);
		foodPerDay = 0;
		maxFoodPerDay = PApplet.map(dnafuret.genes[1], 0, 1, 500, 750);
